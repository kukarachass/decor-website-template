    ...
    <section className="container-x py-8 md:py-12">
      <Reveal>
          <div className="relative grid overflow-hidden rounded-[2.5rem] bg-[linear-gradient(135deg,#F6E8E3_0%,#EFE6D6_45%,#E3EBE2_100%)] lg:grid-cols-[1.05fr_1fr]">
              <div className="relative z-10 min-w-0 px-7 py-12 sm:px-12 sm:py-16">
    <span className="inline-flex items-center gap-2 rounded-full bg-white/70 px-4 py-1.5 text-[0.68rem] font-semibold tracking-[0.16em] text-ink-2 uppercase">
      <Gift className="h-3.5 w-3.5 text-rose" />
        {t.home.bannerEyebrow}
                  ...
                  <h2 className="mt-6 max-w-md break-words font-display text-[2rem] leading-[1.08] sm:text-[2.5rem]">
                      {t.home.bannerTitle}
                  </h2>
              ...
              <div className="relative flex min-w-0 items-end gap-4 px-7 pb-10 sm:px-12 lg:items-center lg:justify-end lg:px-8 lg:py-10">
                  {cards.map((p, i) => (
                      <Link
                          key={p.id}
                          href={`/product/${p.slug}`}
                          className={`group relative min-w-0 flex-1 max-w-[15rem] overflow-hidden rounded-[24px] bg-white/50 p-3 shadow-[0_30px_60px_-40px_rgba(43,37,33,0.6)] backdrop-blur-sm transition-transform duration-500 hover:-translate-y-1 ${
                              i === 0 ? "lg:-mb-8" : "lg:-mt-8"
                          }`}
                      >
                          <ProductArt
