      ...
      >
        <div className="container-x">
          <div className="grid h-16 grid-cols-[1fr_auto_1fr] items-center gap-4 md:h-20">
            ...
            <Link
              ...
              onMouseEnter={() => setMegaOpen(false)}
            >
              <Logo className="hidden w-[150px] text-ink transition-opacity hover:opacity-70 sm:block md:w-[186px]" />
            ...
            <div className="flex items-center justify-end gap-1 md:gap-2">
              ...
              >
                ...
                {hydrated && wishCount > 0 && (
                  <span className="absolute right-1 top-1 h-4 min-w-4 rounded-full bg-rose px-1 text-center text-[0.6rem] font-semibold leading-4 text-paper">
                    {wishCount}
                  </span>
                )}
              </Link>
              ...
              >
                ...
                {accountOpen && (
                  <div className="absolute right-0 top-full w-60 pt-2">
                    <div className="overflow-hidden rounded-2xl border border-line bg-paper p-2 shadow-[0_30px_60px_-30px_rgba(43,37,33,0.45)]">
                      {hydrated && user ? (
                        <>
              ...
              >
                ...
                {hydrated && count > 0 && (
                  <span className="absolute right-0.5 top-1 h-4 min-w-4 rounded-full bg-ink px-1 text-center text-[0.6rem] font-semibold leading-4 text-paper">
                    {count}
                  </span>
                )}
              </button>
        ...
        >
          <div className="container-x grid grid-cols-[1fr_260px] gap-10 py-10">
            <div>
              ...
              <div className="grid grid-cols-3 gap-x-8 gap-y-1">
                {categories.map((c) => (
                  ...
                  >
                    <span className="flex items-center gap-2 font-title text-lg text-ink transition-colors group-hover:text-clay-2">
                      <span
            ...
            >
              <div className="absolute inset-0 bg-[linear-gradient(160deg,#F8EDEB,#E6BFB9)] transition-transform duration-700 group-hover:scale-105" />
              <div className="relative">
                <p className="eyebrow">{t.header.megaFavorite}</p>
                <p className="mt-1 whitespace-pre-line font-display text-2xl leading-tight">
                  {t.header.megaTulips}
                </p>
                <span className="mt-3 inline-block text-[0.8rem] font-medium text-ink-2 underline underline-offset-4">
                  {t.header.megaLook}
                </span>
              </div>
            </Link>
